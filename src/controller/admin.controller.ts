...
// Update Food Item
export const deleteFoodItem = tryCatch(async (req: Request, res: Response) => {
  const {id} = req.params;

  const updatedFoodItem = await FoodItemModel.findByIdAndUpdate(id, {itemStatus: ItemStatus.DELETED}, {new: true});
  if (!updatedFoodItem) {
    throw new NotFoundError("Food Item not found!");
  }
  const response: StandardResponse<string> = {
